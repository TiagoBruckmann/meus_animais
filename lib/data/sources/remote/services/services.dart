...
import 'package:meus_pets/data/sources/local/manager/update_pets.dart';
...
class Services {
  ...
  void deleteAllTokens() async {
    ...
    analytics.logEvent(name: "delete_all_tokens-v1");
  }
  ...
  void deleteOnlyToken( String keyName ) async {
    ...
    analytics.logEvent(name: "delete_only_token-v1");
  }

  uploadPicture( ModelPets modelPets, File picture, context ) async {
  ...
  rateApp() async {
    ...
    analytics.logEvent(name: "rateApp-v1");
    String url;
    if ( Platform.isAndroid ) {
      url = "https://play.google.com/store/apps/details?id=com.sega.sprint";
    } else {
      url = "www.apple.com/br/app-store/";
    }

    if (await canLaunch(url)) {
      await launch(url);
    } else {
      String error = 'Não foi possível ir para a loja $url';
      crash.log(error);
      throw error;
    }
  }
